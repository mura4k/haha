...
def put_text(image, text, position, font_size=32, color=(255, 255, 255)):
    ...
    font = ImageFont.truetype("arial.ttf", font_size)  # Make sure the path is correct
...
def draw_info_text(image, brect, handedness, hand_sign_text, finger_gesture_text):
    ...
    image = put_text(image, info_text, (brect[0] + 5, brect[1] - 30), font_size=20)

    if finger_gesture_text != "":
        image = put_text(image, "Finger Gesture: " + finger_gesture_text, (10, 60), font_size=24)
...
def draw_info(image, fps, mode, number):
    text = f'FPS: {fps:.2f}, Mode: {mode}, Number: {number}'
    image = put_text(image, text, (10, 30), font_size=24)
    return image
